class HanoiDom {
	constructor(width = 30, height = 10, discNum = 3, noStyle = false) {
	...
	elementAdd() {
		...
		// 创建柱子A、B、C
		this.A = new PixelCanvas.Element(pillar, ['background: #ddd', 'color: #ddd', 'border-radius: 2px']);
		this.A.position.y = 1;
		this.B = this.A.clone();
		this.C = this.A.clone();
		...
		this.C.name = 'C';
		this.group = new PixelCanvas.Group();
		// 柱子加入组合，通过整体组合调整位置
		...
		for (let j = 1; j <= this.discNum; j++) {
			discItem = new PixelCanvas.Element([new Array(j * 2 + 1).fill('Θ')], ['background: #007fff', 'color: #007fff', 'border-radius: 5px']);
...
let hanoiDom = new HanoiDom(31, 10, 4);